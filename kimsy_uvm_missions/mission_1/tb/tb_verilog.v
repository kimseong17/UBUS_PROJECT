module tb_verilog;
  ...
  begin
    ...
    @(posedge pclk); penable = 1; pwrite = 1; paddr = addr; pwdata = data;
    wait (pready == 1);
  ...
  begin
    @(posedge pclk); pselx = 1; penable = 0;
    @(posedge pclk); penable = 1; pwrite = 0; paddr = addr;
    wait (pready == 1);
    @(posedge pclk); data = prdata; pselx = 0;
  end
  endtask
  ...
  initial begin
    ...
    for (int i = 0; i < repeat_val; i = i + 1) begin
      ...
      if (pslverr) begin
        ...
        if (temp_data == data_list[i]) begin
          ...
          $display("fail! addr:%h, expected:%h, real:%h",
                   addr_list[i], data_list[i], temp_data);
          err_cnt = err_cnt + 1;
        end
      end
    end
    ...
    if (err_cnt != 0)
      $display("[RESULT] TEST FAILED , err_cnt : %d", err_cnt);
    else
