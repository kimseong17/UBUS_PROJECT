...
module apb_completer #( parameter RW_REGION = 8,
    ...
    //----------------------------------------------
    // Declartion Internal Signals and Variables
    //----------------------------------------------

    logic [DATA_WIDTH - 1:0] mem [2**(RW_REGION) - 1:0];    // register for write/read data
                                                            // it has 32bit of datawidth
                                                            // and amount of address

    logic enable_condition; // choose either using area condition to detect pslverr or not

    logic pre_pready;       // indicates previous pready value
    logic [RW_REGION-1:0] temp_addr; // giving temporary value to addr to avoid combinational region did not converge error
