...
covergroup cov_trans() with function sample(packet trans);
    ...
    trans_addr : coverpoint trans.addr { option.auto_bin_max = 16; }

    // 2. 방향 (read/write): Read=0, Write=1로 분리
    trans_dir : coverpoint trans.read { bins READ = {0}; bins WRITE = {1}; }
    ...
    // 5. 에러 커버리지 (트랜잭션이 에러 없이, 에러와 함께 완료되는지 확인)
    //trans_error : coverpoint trans.error { bins NO_ERROR = {0}; bins HAS_ERROR = {1}; }
    
    // 6. 지연(Wait State) 커버리지 (지연이 없는 경우와 있는 경우 모두 확인)
    trans_wait_state : coverpoint trans.wait_state.size() { bins NO_WAIT = {0}; bins WITH_WAIT = {[1:$]}; }
    
    // 7. 중요 크로스: 방향 X 에러
    // READ 에러와 WRITE 에러가 모두 발생하는지 확인
    //trans_DirXError : cross trans_dir, trans_error;
...
class ubus_coverage extends uvm_subscriber #(packet);
    ...
    virtual function void write(packet t);
        ...
        `uvm_info(get_name(), $sformatf("Sampled transaction: addr=0x%0h, size=%0d", t.addr, t.size), UVM_LOW)
    endfunction
