class my3_vip_slave_test extends uvm_test;
	...
	uvm_config_db#(uvm_object_wrapper)::set(this,"env.s_agent.sequencer.run_phase","default_sequence", slave_sequence::type_id::get()); //master와 다르게 반응형이기 자동으로 실행되는 default sequence를 등록/ 별도의 start 없이도 자동 수행 , uvm_object_wrapper, uvm_config_db는 UVM의 자동화,연결 데이터베이스 
	...
	phase.raise_objection(this);
		for (int i =0; i<10; i++) begin
			packet tr = packet::type_id::create($sformatf("tr_%0d",i));
			assert(tr.randomize());
